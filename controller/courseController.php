...
public function select($args){
		...
		for($x=0;$x<$arraySize;$x++){	
			...
			<tr>
				...
				<td class=\"text-center\">				
					<form method=\"post\" action=\"/course/view\">
	...
	/*** load the index template ***/
		$this->registry->template->show('header');
		$this->registry->template->show('navigation');
		$this->registry->template->show('course_select');
		$this->registry->template->show('footer');
}
...
public function view(){
		...
		if(isset($_POST['Submit'])) { 
			...
			$this->registry->template->depAbr= $dep->collegeAbr;
		}
	//echo $args[0];
	/*** should not have to call this here.... FIX ME ***/
			$this->registry->template->show('header');
		$this->registry->template->show('navigation');
		$this->registry->template->show('course_view');
		$this->registry->template->show('footer');
}
